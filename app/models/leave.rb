class Leave < ActiveRecord::Base
  belongs_to :employee
  validates_presence_of :no_of_days, :reason_of_leave,:employee_id,  :type_of_leave, :start_date_of_leave, :end_date_of_leave, :message => "Can't be blank"
  ...
  def validate
    ...
    if  ( !self.no_of_days.blank? and self.no_of_days > 2  and  self.type_of_leave == "casual" )    
       errors.add( "", " Casual Leave can't take continiously more than two day's ")       
     end
     
    if ( !self.no_of_days.blank?  and self.no_of_days > 2  and  self.type_of_leave == "sick" )    
       errors.add( "", " Sick Leave can't take continiously more than two day's" )     
     end
